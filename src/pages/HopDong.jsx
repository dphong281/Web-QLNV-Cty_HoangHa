...
export default function HopDong() {
  ...
  function handleExportExpiring() {
    ...
    const rows = expiring.map((c) => [c.so_hd_goc || '', c.ma_nv, c.hoTen, c.khoi, c.loaiHdHienThi, c.ngay_ky, c.ngay_het_han, c.luong_co_ban])
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? `Sửa hợp đồng - Mã HĐ hệ thống: ${editing.ma_hd} - Mã HĐ: ${editing.so_hd_goc || '—'}` : 'Ký hợp đồng mới'} wide>
